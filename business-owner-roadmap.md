...
**Current Status:** March 22, 2026 reality check: frontend `main` now includes merged PR **#729** on commit `e79f66b8288911c207ab8022d4bb33d8fd6f407f`, and issue **#728** is closed. `Run Tests` (**23397619626**), `Playwright Tests` (**23397619631**), and `Build and Deploy FE` (**23397619629**) are all green on the actual current head `e79f66b8288911c207ab8022d4bb33d8fd6f407f`, so the earlier deployment-health and documentation-staleness blockers are closed on released `main`. The protected `Strict Backend Sign-off Gate` also executes one real job on the same head (run **23397619628**) and uploads artifact `strict-signoff-report-23397619628`, but the run log still shows `SIGNOFF_API_BASE_URL` and `SIGNOFF_TEST_PASSWORD` were not configured, so the workflow generated a **not-configured** artifact, skipped the strict Playwright suite, and still does **not** provide artifact-backed real-backend release evidence. PR **#729** materially improved operator trust by landing refreshed testing docs plus **620** unit tests across **39** previously uncovered frontend files, but business-owner MVP sign-off remains blocked by protected-environment provisioning and the still-permissive supporting enterprise Playwright suites now tracked in issue **#730** / draft PR **#731**.
...
## MVP Blockers Reality Check (March 22, 2026 - post merge of PR #729)

### Evidence Reviewed

- Recent release-trust work on `main` now includes merged PR **#729**, which refreshed `docs/testing/PLAYWRIGHT_STATUS.md`, refreshed `docs/implementations/MVP_SIGNOFF_READINESS_BLOCKER_MAPPING.md`, added **620** unit tests across **39** previously uncovered frontend files, and closed issue **#728** on the released branch.
- Latest `Run Tests` on `main` is green on commit `e79f66b8288911c207ab8022d4bb33d8fd6f407f` (run **23397619626**, `success`).
- Latest `Playwright Tests` on `main` is green on the same head (run **23397619631**, `success`).
- Latest `Build and Deploy FE` on `main` is also green on the same head (run **23397619629**, `success`), so the earlier `@vitest/ui` / `vitest` / `@vitest/coverage-v8` deployment blocker remains closed on released `main`.
- The protected strict workflow still executes one real job on current head and uploads artifact `strict-signoff-report-23397619628` (run **23397619628**, `success`), so the prior zero-job bootstrap failure remains closed.
- However, the job log for **23397619628** shows `SIGNOFF_API_BASE_URL` and `SIGNOFF_TEST_PASSWORD` were **not set**; the workflow therefore generated a `mode: "not-configured"` artifact, skipped the strict Playwright suite, and explicitly states "This run is NOT credible release evidence." The business-owner sign-off blocker is now configuration / protected-environment readiness, not workflow scheduling.
...
- `e2e/helpers/auth.ts` still keeps the default corpus materially more permissive than the strict lane: **51** spec files use `withAuth()`, **12** use `loginWithCredentials()`, and only **2** use `loginWithCredentialsStrict()`.
- Broader enterprise evidence suites remain supporting evidence rather than final release proof: current `main` still contains **293** broad `suppressBrowserErrors()` references across **69** spec files, **73** `waitForTimeout()` references across **46** spec files, and **43** `test.skip()` / `test.describe.skip()` calls across **22** spec files.
- The highest-value enterprise workflows called out by the roadmap (`e2e/investor-compliance-onboarding.spec.ts`, `e2e/compliance-reporting-workspace.spec.ts`, `e2e/release-evidence-center.spec.ts`, `e2e/reporting-command-center.spec.ts`, `e2e/compliance-operations-cockpit.spec.ts`, and `e2e/case-drill-down.spec.ts`) are now the explicit hardening target in issue **#730** and draft PR **#731**.
- Public testing and blocker docs are no longer stale on `main`; that blocker is closed. The remaining frontend trust gap is the absence of backend-confirmed strict evidence plus the realism debt of the broader enterprise suites.

### Blocker Validation Status

- 🟡 **ARC76 critical-path blocker is closed in code and operationally improved:** `mvp-backend-signoff.spec.ts` + `loginWithCredentialsStrict()` remain fail-closed, and the dedicated strict workflow still schedules a job and uploads an artifact on current `main`. However, the latest artifact is still `not-configured`, so there is still no `is_release_evidence:true` run on current head.
...
- ✅ **Stale stakeholder-doc blocker is closed on current `main`:** PR **#729** refreshed `docs/testing/PLAYWRIGHT_STATUS.md` and `docs/implementations/MVP_SIGNOFF_READINESS_BLOCKER_MAPPING.md`, aligned them to current workflow evidence, and added auditable unit-coverage backing for release-trust claims.
...
- 🟡 **Enterprise-workflow realism remains the next frontend trust milestone:** issue **#730** and draft PR **#731** target the most commercially important supporting suites by replacing permissive auth, broad error suppression, and fixed sleeps with backend-backed auth attempts, semantic waits, and clearer degraded-state assertions.
...
Current Playwright coverage now satisfies most of the important **frontend/code-level** MVP blocker criteria: wallet-free auth-first routing is well covered, the canonical strict backend-auth / deployment suite is fail-closed, legacy route drift is contained, shell/mobile parity is proven, procurement-grade axe coverage is green for the highest-value enterprise journeys, the release-evidence truthfulness and documentation-hardening work through **#727** and merged PR **#729** is now on `main`, and the current head is green in `Run Tests`, `Playwright Tests`, and `Build and Deploy FE`. The strict workflow infrastructure story also remains materially improved because run **23397619628** schedules a real job and uploads an audit artifact. The corpus still does **not** satisfy the full **business-owner** blocker because (1) the latest strict artifact is explicitly `mode: "not-configured"` rather than backend-confirmed release evidence, (2) the protected Playwright lane did not execute against a live backend on the current head, and (3) the broader green corpus remains materially more permissive than production reality in several enterprise-critical suites. That means the Playwright corpus is now materially closer to MVP sign-off than the previous roadmap stated, and the stale-docs blocker is closed, but one protected-environment proof gap plus supporting-suite realism debt still remain before final business-owner sign-off.
...
2. Trigger a `workflow_dispatch` strict run after secrets are provisioned, download the resulting artifact, and verify `signoff-status.json` flips from infrastructure-only proof to backend-confirmed release evidence on the current head.
...
5. Merge or supersede draft PR **#731** so the highest-value enterprise suites (`e2e/investor-compliance-onboarding.spec.ts`, `e2e/compliance-reporting-workspace.spec.ts`, `e2e/release-evidence-center.spec.ts`, `e2e/reporting-command-center.spec.ts`, `e2e/compliance-operations-cockpit.spec.ts`, `e2e/case-drill-down.spec.ts`) move away from `withAuth()` + broad `suppressBrowserErrors()` + fixed sleeps toward backend-backed auth attempts, tighter failure surfacing, semantic waits, and clearer live-data / degraded-state assertions.
6. Remove the remaining fixed sleeps in blocker-adjacent assertions (for example 3s unauthenticated redirect waits and 300ms / 1000ms UI-toggle waits) in favor of user-observable conditions.
7. Continue shrinking broad console / page-error suppression, seeded-auth usage, and remaining `waitForTimeout()` hotspots in secondary flows so ordinary CI evidence becomes materially closer to production reality.
8. Preserve the now-green automated accessibility evidence for Home, Guided Launch, Compliance, Compliance Setup Workspace, whitelist management, Team Workspace, shell-adjacent patterns, release evidence, and the newer reporting / onboarding surfaces so accessibility claims remain procurement-grade evidence instead of a one-off artifact.
...
- **HIGH:** Close issue **#730** by merging or superseding draft PR **#731** so the six highest-value enterprise evidence suites become materially less permissive and more production-relevant.
- **HIGH:** Keep the refreshed public Playwright status / blocker docs current now that PR **#729** has landed on `main`; do not allow the roadmap, blocker mapping, and Playwright status summaries to drift again.
...
- **Hold MVP Foundation at 75% for now:** current head is green in `Run Tests`, `Playwright Tests`, and `Build and Deploy FE`, PR **#729** improved auditable evidence and unit coverage materially, and the strict gate still schedules and uploads an artifact. However, there is still no backend-confirmed `is_release_evidence:true` run on current `main`, so this is not yet the moment for another confidence bump.
- **Close the stale-docs blocker on released `main`:** PR **#729** landed the overdue Playwright status refresh and blocker mapping refresh, so roadmap discussions should now treat documentation drift as a maintenance concern rather than an open MVP blocker.
- **Treat issue #730 / draft PR #731 as the active frontend trust milestone:** the next material gap is no longer public-doc freshness or zero-job workflow execution; it is protected-environment proof plus realism hardening for the most commercially important enterprise suites.
- **Keep accessibility as a maintained trust asset:** the navigation / keyboard blocker and contrast blocker are no longer the main gaps. The remaining priority is sustaining that evidence and tying it to a truly backend-confirmed strict release artifact.
...
3. 🟡 **Harden highest-value enterprise evidence suites** - Merge or supersede the issue #730 / PR #731 work so onboarding, reporting, release evidence, cockpit, and case-drill-down flows better reflect backend-backed auth, semantic waits, and truthful degraded-state behavior
...
- Documentation drift: the stale-docs blocker is closed on `main` after PR **#729**, but roadmap, blocker mapping, and Playwright status summaries still need to stay in sync with each new strict-signoff or enterprise-hardening milestone

---

**Last Updated:** March 22, 2026 (reality check for current head `e79f66b`; `Run Tests` **23397619626**, `Playwright Tests` **23397619631**, and `Build and Deploy FE` **23397619629** are green on `main`; `Strict Backend Sign-off Gate` **23397619628** still executes a real job and uploads an artifact but the run is still `mode: "not-configured"` because `SIGNOFF_API_BASE_URL` / `SIGNOFF_TEST_PASSWORD` are not set, so the strict Playwright suite is skipped and there is still no backend-confirmed release-evidence artifact; issue **#728** is closed by merged PR **#729**; issue **#730** and draft PR **#731** are the active frontend trust-hardening milestone; backend issue **scholtz/BiatecTokensApi#589** remains closed on API `master`; and the remaining business-owner blocker is protected-environment provisioning plus supporting-suite realism in the highest-value enterprise Playwright flows)
