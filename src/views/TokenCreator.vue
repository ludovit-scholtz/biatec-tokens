  ...
  <div class="min-h-screen px-4 py-8">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="text-center mb-8">
        <h1 class="text-4xl font-bold text-white mb-4">Create New Token</h1>
        <p class="text-gray-300 text-lg">Choose your token standard and deploy in seconds</p>
      </div>

      <!-- Token Standard Selection -->
      <div class="glass-effect rounded-xl p-6 mb-8">
        <h2 class="text-2xl font-semibold text-white mb-6">Select Token Standard</h2>
          ...
          >
            <div class="flex items-center space-x-3 mb-3">
              <div class="w-10 h-10 rounded-lg flex items-center justify-center" :class="standard.bgClass">
                <i :class="standard.icon + ' text-white'"></i>
              </div>
              <div>
      ...
      <div v-if="selectedStandard" class="glass-effect rounded-xl p-6">
        <h2 class="text-2xl font-semibold text-white mb-6">Token Details</h2>
        <form @submit.prevent="createToken" class="space-y-6">
          ...
          <div>
            ...
            <div class="flex space-x-4">
              <label class="flex items-center space-x-2">
                ...
                />
                <span class="text-white">Fungible Token (FT)</span>
              </label>
              <label class="flex items-center space-x-2">
                <input
                  v-model="tokenForm.type"
                  ...
                  class="w-4 h-4 text-biatec-accent focus:ring-biatec-accent"
                />
                <span class="text-white">Non-Fungible Token (NFT)</span>
              </label>
            </div>
          </div>
          ...
          <div v-if="tokenForm.type === 'NFT'" class="space-y-4">
            <div class="flex items-center justify-between">
              ...
              <button
                type="button"
                @click="addAttribute"
                class="px-3 py-1 bg-biatec-accent/20 text-biatec-accent rounded-lg hover:bg-biatec-accent/30 transition-colors text-sm"
              >
            ...
            <div v-for="(attr, index) in tokenForm.attributes" :key="index" class="flex space-x-2">
              <input
                ...
                class="flex-1 px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-biatec-accent"
              />
              <input
                v-model="attr.value"
                type="text"
          ...
          <div class="flex justify-end">
            <button
              type="submit"
              :disabled="isCreating"
              class="btn-primary px-8 py-3 rounded-xl text-white font-semibold flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
const tokenStandards = [
  {
    ...
    description: 'Native Algorand fungible and non-fungible tokens with built-in metadata support.',
    icon: 'pi pi-circle-fill text-lg',
    bgClass: 'bg-blue-500'
  },
  {
    name: 'ARC200',
    type: 'Algorand ERC20 Equivalent',
    description: 'Algorand smart contract tokens compatible with ERC20 standards.',
    icon: 'pi pi-coins text-lg',
    bgClass: 'bg-green-500'
  },
  {
    name: 'ARC72',
    type: 'Algorand ERC721 Equivalent',
    description: 'Algorand NFT standard with Ethereum-style functionality.',
    icon: 'pi pi-image text-lg',
    bgClass: 'bg-purple-500'
  },
  {
    name: 'ERC20',
    type: 'Ethereum Fungible Token',
    description: 'Standard Ethereum fungible tokens with full EVM compatibility.',
    icon: 'pi pi-ethereum text-lg',
    bgClass: 'bg-yellow-500'
  },
  {
    name: 'ERC721',
    type: 'Ethereum NFT',
    description: 'Ethereum non-fungible tokens with rich metadata and ownership tracking.',
    icon: 'pi pi-crown text-lg',
    bgClass: 'bg-pink-500'
  }
]
