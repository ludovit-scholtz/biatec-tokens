 ...
 */
export async function retryWithBackoff<T>(operation: () => Promise<T>, config: RetryConfig = DEFAULT_RETRY_CONFIG, onRetry?: (attempt: number, error: Error) => void): Promise<T> {
...
export function getTroubleshootingSteps(errorType: WalletErrorType): string[] {
  const troubleshooting: Record<WalletErrorType, string[]> = {
    [WalletErrorType.PROVIDER_NOT_FOUND]: ["Ensure your wallet extension is installed and enabled", "Try refreshing the page", "Check if your wallet supports this network"],
    [WalletErrorType.CONNECTION_REJECTED]: ["Open your wallet and approve the connection request", "Make sure your wallet is unlocked", "Try connecting again"],
    [WalletErrorType.CONNECTION_TIMEOUT]: ["Check your internet connection", "Make sure your wallet app is responding", "Try again in a few moments"],
    [WalletErrorType.NETWORK_SWITCH_FAILED]: ["Verify the network is supported by your wallet", "Try manually switching networks in your wallet", "Refresh and reconnect"],
    [WalletErrorType.NETWORK_MISMATCH]: ["Switch to the correct network in your wallet", "The UI network must match your wallet network", "Try disconnecting and reconnecting"],
    [WalletErrorType.WALLET_LOCKED]: ["Unlock your wallet", "Enter your password in the wallet app", "Try again after unlocking"],
    [WalletErrorType.PERMISSION_DENIED]: ["Grant the necessary permissions in your wallet", "Check wallet settings for blocked sites", "Try resetting permissions and reconnecting"],
    [WalletErrorType.UNKNOWN]: ["Try refreshing the page", "Check your wallet app for errors", "Contact support if the issue persists"],
  };
