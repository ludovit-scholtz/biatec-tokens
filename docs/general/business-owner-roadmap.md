...
- **URGENT:** Restore fresh current-head executable evidence for the maintained release branch. Without that, the business cannot distinguish “historically green” from “currently sign-offable.”
...
- **Lower evidentiary confidence, not product ambition:** the product surface still looks materially stronger than it did earlier in the quarter, but the business no longer has fresh executable evidence available from this environment for the exact head being reviewed.
- **Treat historical upstream run IDs as supporting context, not current proof:** until the source repository is reachable again or the maintained repo exposes equivalent workflow evidence, those run IDs should not be used as sign-off-grade citations.
...
- Documentation drift: the roadmap is current again, but `PLAYWRIGHT_STATUS.md`, `MVP_SIGNOFF_READINESS_BLOCKER_MAPPING.md`, and parts of `e2e/README.md` still do not match the April 13, 2026 reality check. They overstate strict-lane freshness, blur the maintained fork vs source-repo evidence boundary, and still lag the real `/launch/workspace` → `/launch/guided` launch-route contract
